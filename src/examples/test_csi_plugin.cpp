...
public:
  Flags()
  {
    ...
    add(&Flags::create_parameters,
        ...
        "specified as a semicolon-delimited list of key:value paris.\n"
...
int main(int argc, char** argv)
{
  ...
  if (flags.volumes.isSome()) {
    foreachpair (const string& name,
      ...
      if (name.find_first_of(os::PATH_SEPARATOR) != string::npos) {
        error =
          string("Volume name cannot contain '") + os::PATH_SEPARATOR + "'";
      ...
      if (error.isSome()) {
        cerr << "Failed to parse the '--volumes' flag: " << error->message
             << endl;
        return EXIT_FAILURE;
      }
    }
