...
{
  LOG(INFO) << request.GetDescriptor()->name() << ": "
...
public:
      ...
      csi::NodePublishVolumeResponse* response) override
  {
    ...
    if (isSupportedVersion(request, response)) {
      ...
      Try<string> mount = os::shell(
          "mount %s %s %s %s",
          !capability.mount().fs_type().empty()
            ? ("-t " + capability.mount().fs_type()).c_str() : "",
          strings::join(" ", capability.mount().mount_flags()).c_str(),
          id.c_str(),
          request->target_path().c_str());
      ...
      csi::NodeUnpublishVolumeResponse* response)
  {
    ...
    if (isSupportedVersion(request, response)) {
      ...
      Try<string> unmount =
        os::shell("umount %s", request->target_path().c_str());
