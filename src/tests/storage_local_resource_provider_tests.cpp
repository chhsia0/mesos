...
public:
  ...
  void setupResourceProviderConfig(
      ...
      const Option<string> parameters = None())
  {
...
TEST_F(StorageLocalResourceProviderTest, AgentRegisteredWithNewId)
{
  ...
  // NOTE: Instead of expecting a preprovisioned volume, we expect a storage
  // pool of profile "test1" as an indication that the profile is known to the
  // resource provider. Thes same offer should also contain the preprovisioned
  // volume. However, an extra offer containing only the storage pool will be
  // received as a side effect of this workaround, so we just decline it.
  EXPECT_CALL(sched, resourceOffers(&driver, OffersHaveAnyResource(
      std::bind(isStoragePool, lambda::_1, "test1"))))
    .WillOnce(FutureArg<1>(&slaveRecoveredOffers))
  ...
  ASSERT_EQ(
      preprovisioned.disk().source().id(),
      recovered.disk().source().id());
