...
TEST_F(StorageLocalResourceProviderTest, CreateDestroyPreprovisionedVolume)
{
  ...
  // Decline unwanted offers. The master can send such offers before the
  // resource provider receives profile updates, or after exercising operations.
  EXPECT_CALL(sched, resourceOffers(&driver, _))
    .WillRepeatedly(DeclineOffers(declineFilters));
  ...
  // operation would succeed but the second one would fail.
  Future<set<OperationState>> createDiskOperationStates =
    process::collect(
        FUTURE_PROTOBUF(UpdateOperationStatusMessage(), _, _),
        FUTURE_PROTOBUF(UpdateOperationStatusMessage(), _, _))
  ...
  EXPECT_CALL(sched, resourceOffers(&driver, AllOf(
      OffersHaveAnyResource(
          std::bind(isMountDisk<Resource>, lambda::_1, "test")),
      OffersHaveAnyResource(isPreprovisionedVolume<Resource>))))
    .WillOnce(FutureArg<1>(&offers));
  ...
  set<OperationState> expectedOperationStates = {
    OperationState::OPERATION_FINISHED, OperationState::OPERATION_FAILED};
  ...
  Resource unimported = *Resources(offer.resources())
    .filter(isPreprovisionedVolume<Resource>)
    .begin();

  // Destroy the imported and unimported volumes.
  Future<set<OperationState>> destroyDiskOperationStates =
    process::collect(
        FUTURE_PROTOBUF(UpdateOperationStatusMessage(), _, _),
        FUTURE_PROTOBUF(UpdateOperationStatusMessage(), _, _))
      .then([](const std::tuple<
  ...
  driver.acceptOffers(
      {offer.id()}, {DESTROY_DISK(imported), DESTROY_DISK(unimported)});

  expectedOperationStates = {OperationState::OPERATION_FINISHED};
  ...
  EXPECT_SOME_EQ(Gigabytes(5), raw.disk());
