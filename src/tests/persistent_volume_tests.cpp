...
// because we intend to keep `GROW_VOLUME` non-speculative.
TEST_P(PersistentVolumeTest, NonSpeculativeGrowAndLaunch)
{
  ...
  // The create and grow calls will succeed, but launch calls will be dropped
  // with `TASK_ERROR`.
  driver.acceptOffers(
      {offer.id()},
...
// volume, because we intend to keep `SHRINK_VOLUME` non-speculative.
